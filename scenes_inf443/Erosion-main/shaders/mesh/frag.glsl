...
uniform sampler2D pierre;
uniform sampler2D neige;
...
void main()
{
	...
	if(diffuse_component>0.0){
		...
		if (fragment.position[2]>h0 + 0.005*sin(80*fragment.position[0]+ 80*fragment.position[1]) && (dot(N,vec3(0,0,1))*(fragment.position[2] - h0)>2.0f || dot(N,vec3(0,0,1))*(fragment.position[2] - h0)<-2.0f)){ //+ 0.1*sin(100*fragment.position[0]+ 100*fragment.position[1]) && dot(N,vec3(0,0,1))>0.4
	...
	// 2nd texture
	vec4 couleur_pierre = texture(pierre,uv_image);

	//adaptation a la pente
	float melangeur = dot(N,(vec3(0,0,1)));
	if (melangeur<0){
		melangeur = -1*melangeur;
	}
	if (melangeur<0.2){
		color_image_texture = couleur_pierre;
	}
	else if (melangeur<=0.8 && melangeur>=0.2){
		melangeur = (melangeur-0.2)/0.6;
	color_image_texture = (melangeur)*color_image_texture + (1-melangeur)*couleur_pierre;
	}	

	//3rd texture
	vec4 couleur_neige = texture(neige,uv_image);
	

	if (fragment.position[2]>h0 + 0.005*sin(80*fragment.position[0]+ 80*fragment.position[1]) && (dot(N,vec3(0,0,1))*(fragment.position[2] - h0)>2.0f || dot(N,vec3(0,0,1))*(fragment.position[2] - h0)<-2.0f)){ //+ 0.1*sin(100*fragment.position[0]+ 100*fragment.position[1]) && dot(N,vec3(0,0,1))>0.4
		color_image_texture = couleur_neige;
	}
	...
	vec3 color_object  =  material.color * color_image_texture.rgb;//fragment.color *
